...
export function setup() {
    ...
    //authorization in the System as employee/test
     let auth = http.post("http://localhost:8080/login?username=employee&password=test");
     
     //orders API, get length
    let api_id_orders_test = http.get('http://localhost:8080/api/orders/');
    ...
    elements_number = elements_number.replace(/\"/g, "");
     ...
     let api_id_orders = http.get('http://localhost:8080/api/orders/?size=' + elements_number);
...
{ 
    group('PART 2: Accepting orders.', function() 
    {
        ...
        let get_json = http.get("http://localhost:8080/api/orders/" + array_of_ids[i]);
